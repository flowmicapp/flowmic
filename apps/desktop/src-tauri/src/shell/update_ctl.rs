...
pub fn update_check(base: String, state: State<'_, UpdateState>) -> UpdateStateDto {
    ...
    // worker, and the body blocks on the manifest fetch. See `socket::blocking`.
    run_blocking(|| {
        {
...
pub fn update_download(app: AppHandle, state: State<'_, UpdateState>) -> UpdateStateDto {
    // P0 2026-09-03 — `#[tauri::command(async)]` puts this on a tokio
    // worker, and the body blocks for MINUTES (download + sha256 + unzip). See `socket::blocking`.
    run_blocking(|| {
        let artifact = {
            let inner = state.inner.lock().unwrap_or_else(|e| e.into_inner());
            match &inner.plan {
...
pub fn update_apply(app: AppHandle, state: State<'_, UpdateState>) -> UpdateStateDto {
    // P0 2026-09-03 — `#[tauri::command(async)]` puts this on a tokio
    // worker, and the body blocks on staging the installer before exiting. See `socket::blocking`.
    run_blocking(|| {
        let (pkg_path, kind) = {
            let inner = state.inner.lock().unwrap_or_else(|e| e.into_inner());
