...
try {
  ...
  //     v24.15.0 there; docs/archive/strategy/2026-08-05-local-crosscheck-of-remote-window.md
  ...
  //     (src-tauri/src/sidecar/node_runtime.rs, resolve_node_exe) prefers the bundled copy
...
// The built executable's path is per-platform. macOS verifies the copy inside
// the `.app`; Linux's AppImage/deb builders consume the extensionless release
// binary; Windows consumes the `.exe`. Pointing Linux at `.exe` made every real
// Linux build fail after it had already produced the correct binary.
// Cargo and Tauri both honor CARGO_TARGET_DIR. The Linux validation lane keeps
// build output on F:, so looking only under src-tauri/target would verify an
// unrelated stale binary (or fail after a successful build).
const cargoTargetDir = resolve(process.env.CARGO_TARGET_DIR ?? join(ROOT, 'src-tauri', 'target'));
...
const exePath =
  process.platform === 'darwin'
    ...
    : process.platform === 'win32'
      ...
      : join(releaseDir, 'flowmic-desktop');
let exe;
try {
