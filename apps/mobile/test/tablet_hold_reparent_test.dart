...
void main() {
  testWidgets(
    '🔴 WP8-P0: a hold started on the two-column tablet dock still ends when '
    'the pointer lifts — the arrangement flip must not orphan the gesture',
    (WidgetTester tester) async {
      final ChatController controller =
          await _pumpPage(tester, width: 800, height: 900);

      // Preflight: this IS the two-column dock (the reparent precondition).
      ...
      // The hold was accepted AND the arrangement flipped: recording hides
      // the key group at every width (S5) — this rebuild is the reparent.
      expect(controller.isRecording, isTrue,
          reason: 'harness: the press was not accepted — nothing to prove');
      expect(_group, findsNothing,
          reason: 'S5: recording must hide the key group (the flip happened)');

      // Now the other half of the gesture. On the broken build these pointer
      // events go to a disposed element and the FSM stays RECORDING forever
      // (the device ran 3:40 across 8 rolling segments before force-stop).
      ...
      expect(controller.isRecording, isFalse,
          reason: '🔴 the lift never reached the controller — the tablet '
              'arrangement flip orphaned the in-flight hold (WP8-P0)');

      // And the dock is whole again: idle two-column, bar back to idle face.
      expect(_group, findsOneWidget,
          reason: 'idle again ⇒ the two-column dock is back');
