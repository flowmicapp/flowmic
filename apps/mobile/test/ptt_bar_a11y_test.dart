...
void main() {
  group('U12 — semantics reach the tree, not just the widget', () {
    testWidgets('idle: labelled + exposed as a tappable button', (
      ...
      // Tristate folds the old (hasEnabledState, isEnabled) flag pair into
      // one value: isTrue = the node HAS an enabled state and it is on.
    ...
    testWidgets('processing: disabled state has NO tap action wired', (
      ...
      final SemanticsNode node = tester.getSemantics(find.byType(PttBar));
      final SemanticsData data = node.getSemanticsData();
      // Tristate.isFalse = the node HAS an enabled state and it is off —
      // exactly the old hasEnabledState=true + isEnabled=false pair.
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final SemanticsNode node = tester.getSemantics(find.byType(PttBar));
        // The node's own attached owner — same object the deprecated
        // `tester.binding.pipelineOwner.semanticsOwner` used to hand back,
        // reached without the deprecated binding accessor.
