...
// (The 「+」 pair has since moved again, verbatim, to chat_flow_plus_panel.dart.)
//
...
Widget _recordingStripRouted(_ChatFlowPageState s, AppStrings strings) =>
    RecordingPanel(
      ...
      strings: strings,
      // NR-96-B — the session's one answer; never re-derived here.
      engineReconnect: s.controller.session.engineReconnect.value,
    );
...
PttVisual _pttVisualRouted(_ChatFlowPageState s) {
  ...
  // Card RC4 — the user's press, never a recovery pass on the wire
  // (`pressSessionState`); a pass that cannot yield shows the busy face.
  if (s.controller.recoveryRefusesPress) return PttVisual.processing;
  return switch (s.controller.pressSessionState) {
