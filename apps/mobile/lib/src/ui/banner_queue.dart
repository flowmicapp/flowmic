...
// The banner ID registry moved out VERBATIM at the 800-line cap (card MP-14).
// Re-exported rather than merely imported, so every existing `import
// 'banner_queue.dart'` still reaches `BannerIds` and not one call site had to
// be touched by the split.
export 'banner_ids.dart';
...
BannerQueue buildChatBanners({
  ...
  /// or by its auto-hide window, which fire the same callback).
  ControlKeyRefusal? controlKeyRefusal,
  ...
  void Function()? onDismissControlKeyRefusal,
  ...
  // Card MP-14: a control key the far end would not run. BLOCKING for the same
  // reason a failed ➤ is (an act the user asked for did not happen), and
  // dismissible for the same reason too — it describes a press that is over, not
  // a live trap. It carries NO action: there is nothing this phone can retry on
  // the user's behalf, because the next press has to go somewhere else, or after
  // they click into a box, or simply again; offering 「重发」("resend") would
  // promise a fix this end cannot perform.
  if (controlKeyRefusal != null) {
    queue.push(
      BannerItem(
        id: BannerIds.controlKeyRefused,
        severity: BannerSeverity.blocking,
        message: controlKeyRefusalText(strings, controlKeyRefusal),
        dismissible: true,
        onAction: onDismissControlKeyRefusal,
      ),
    );
  }
