...
import 'article_page.dart' show ArticlePage;
...
part 'history_page_selection.dart';
...
class _HistoryPageState extends State<HistoryPage> {
  ...
  /// Card CR-12-G — the recordings in [_hits] (each stands there as its head
  /// row), by articleId, and the words that found them. Read only while
  /// [_hits] is non-null.
  Map<String, ArticleSearchHit> _articleHits =
      const <String, ArticleSearchHit>{};
  String _hitQuery = '';
  ...
  Future<void> _runSearch(String raw) async {
    ...
    // CR-12-G: a recording's matching parts become ONE result (search_hits.dart).
    final SearchResults results = groupSearchHits(
      found,
      articleRows: await articleRowsOnDisk(widget.store, articleIdsOf(found)),
      query: q,
    );
    if (!mounted) return;
    setState(() {
      _hits = results.rows;
      _articleHits = results.articles;
      _hitQuery = q;
    });
  }

  /// Card CR-12-G — a recording in the search results. Same two gestures the
  /// chat page gives its recording card: tap opens, long press is the menu.
  Widget _articleHitCard(
    BuildContext context,
    AppStrings s,
    ArticleSearchHit hit,
  ) => SearchArticleHitCard(
    hit: hit,
    query: _hitQuery,
    strings: s,
    onOpen: (ArticleSearchHit h) => unawaited(_openArticleHit(context, s, h)),
    onLongPress: _selection.active
        ? null
        : (TimelineEntry e) => unawaited(_onLongPress(context, e, s)),
  );

  /// Opens the recording at the paragraph holding its first match. Members are
  /// read from storage: a search hit need never have been paged in.
  Future<void> _openArticleHit(
    BuildContext context,
    AppStrings s,
    ArticleSearchHit hit,
  ) async {
    final List<TimelineEntry> rows =
        await articleMembersOnDisk(widget.store, hit.head.articleId!);
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (BuildContext context, int i) {
                            ...
                            final ArticleSearchHit? hit =
                                searching ? _articleHits[entry.articleId] : null;
                            if (hit != null && entry.isArticle) {
                              return _articleHitCard(context, s, hit);
                            }
