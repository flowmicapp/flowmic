...
// 800-line cap: connection, mode and badge widgets moved out VERBATIM.
part 'status_badge_widgets.dart';
...
///
/// This presentation projection is computed by [deliveryFaceOf].
/// History retains four states; Linux submission uncertainty is projected from
/// cached plus its verdict code under the lead ruling dated 2026-09-22.
/// window-B3-2b widened this to SEVEN, window-B4-5 to EIGHT. Every addition is
/// a projection of a fact carried by the row (or its queue item). The Linux
/// work adds verdict codes and a control receipt reason, but no history state. [noFocus] reads the SAME `failureReason` string
...
DeliveryFace deliveryFaceOf(TimelineEntry entry, {required bool queued}) {
  if (entry.status == EntryStatus.cached && entry.cachedByVerdict &&
    ...
    return DeliveryFace.injectionUncertain;
  }
  switch (entry.status) {
...
StatusMeta deliveryFaceMeta(DeliveryFace face, AppStrings strings) {
  switch (face) {
    ...
    case DeliveryFace.injectionUncertain:
      return StatusMeta(
        FlowMicColors.amber,
        null,
        '?',
