...
// ⚠️ `isAccountRestricted` used to be imported HERE, for `toOpsUser`. That
// family moved to ./user-ops-view.ts (a verbatim move, 2026-08-31, to get this
// file back under the 800-line cap) and took the import with it — the reasoning
// that justified a repo reaching into auth/ travelled with the code it was
// about. This file no longer converts `restricted_at` into anything: it carries
// the raw column, exactly as its field doc below says.
...
export interface UserRepo {
  ...
  /**
   * 2026-08-31 multi-node — land an account row that came from the writer, on a
   * replica, so a `pc_devices` / `mobile_pairings` row read through ahead of the
   * replication pull can satisfy its foreign key. Sole caller:
   * node/token-rows.ts `applyTokenResolution`.
   *
   * 🔴 `ON CONFLICT(id) DO NOTHING` — IT CREATES A MISSING ROW AND NEVER TOUCHES
   * ONE THAT IS HERE, and that is a different rule from its two siblings
   * (`PcRepo.upsertReplicated` / `MobileRepo.upsertReplicated`, both DO UPDATE).
   * The asymmetry is the point, not an inconsistency:
   *
   *   · those two rows ARE the subject of the request — a token was presented
   *     and they are what it means, so correcting a stale copy is the job;
   *   · this row is a DEPENDENCY DRAGGED IN to satisfy a foreign key. Nobody
   *     asked about it, and the only thing the read-through needs from `users`
   *     is that the id exists. Rewriting it would make an unrelated phone's
   *     handshake a writer of somebody's account row — and this node does serve
   *     the HTTP account routes, so a local write from the last 30 seconds is
   *     something this path would be reverting on another actor's behalf.
   *
   * The replication pull stays the single authority on this table's CONTENT and
   * reconciles within 30 seconds either way, so the choice costs nothing and the
   * blast radius is the whole of the difference.
   *
   * ⚠️ The conflict target is `id` and not a bare `ON CONFLICT`: an `email` or
   * `google_sub` collision under a DIFFERENT id is a real problem, and it must
   * throw so the caller refuses, exactly as its two siblings do. A bare clause
   * would swallow it and leave the FK to fail one statement later, three layers
   * from the cause.
   *
   * ⚠️ FULL ROW, `password_hash` INCLUDED. That follows this file's own
   * precedent rather than contradicting it: {@link UserPage} says in as many
   * words that a repo hands out whole records and that the structural exclusion
   * belongs in the PROJECTION ({@link OpsUserView}, "ABSENT BY CONSTRUCTION"),
   * because that surface shows a row to a PERSON. This one hands a row to
   * another FlowMic node's sqlite file over the channel that already serves
   * `/api/node/snapshot` — the entire `users` table, every hash included. See
   * node/token-rows.ts for the full argument and for why omitting the column
   * would be actively worse (it is nullable, so it would land as NULL).
   */
  upsertReplicated(row: UserRecord): void;
  listAll(): UserRecord[];
  /**
  ...
  return {
    ...
    upsertReplicated(row): void {
      upsertReplicatedStmt.run(
        ...
        row.plan,
        row.locale,
        // Back to the INTEGER the column actually is. `toRecord` is the one
        // place these become booleans; this is the one place they go back, and
        // it is the same `=== 1` convention read in reverse.
        row.is_admin ? 1 : 0,
        row.permanent_free ? 1 : 0,
