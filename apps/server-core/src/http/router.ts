...
import { tryHandleSubscriptionCheckoutRoutes } from './subscription-checkout-routes';
import { tryHandleConsoleRoutes } from './console-routes';
import { tryHandleOpsRoutes } from './ops-routes';
import { tryHandleSaasOpsFamily } from './router-ops-mounts';
...
export function makeHttpHandler(deps: HttpDeps): (req: IncomingMessage, res: ServerResponse) => boolean {
  ...
  return (req, res): boolean => {
    ...
    // 🔴 STARTING a subscription, as opposed to cancelling one. Mounted on its
    // own dep so that turning selling off cannot take the cancel controls down
    // with it — the shape of the 2026-08-30 P0, where one provider's switch
    // silently unmounted another provider's webhook.
    if (deps.subscriptionCheckout && tryHandleSubscriptionCheckoutRoutes(req, res, deps.subscriptionCheckout)) return true;
    ...
    // 🔴 THE SIX saas-only OPERATOR MOUNTS LIVE IN ./router-ops-mounts.ts.
    // They moved there on 2026-09-07 because this file stood at exactly the
    // 800-line cap (verify/lint/file-size.mjs) and the REQ-002 pair could not be
    // added without crossing it — the same forced split that produced
    // ops-refund-release-routes.ts and node-routes-forward-sync.ts. The move was
    // VERBATIM, comments included, and the mode re-check went with it: that
    // second condition is the one keeping a mis-wired dep from opening a
    // cross-account WRITE on a standalone LAN box, and it is argued in full over
    // there rather than summarised here.
    if (tryHandleSaasOpsFamily(req, res, config.mode, deps)) return true;
