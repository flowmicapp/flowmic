...
describe('F6: the writer prunes its forward-ledger dedup table on a real timer', () => {
  ...
  it('REVERSE CONTROL — a SINGLE-NODE deployment arms no such timer at all', async () => {
    ...
    // recovery sweep + card M4-01's anonymous-row cleanup share this override
    // (see retention-cleanup.test.ts's own "arms the sweep" test for that
    // baseline) — a FIFTH timer here would mean this card's sweep is arming even
    // where there is no ledger to prune.
    //
    // ⚠️ THE COUNT ALONE IS NOT THE ASSERTION, and after PR-2 it could not be:
    // 「there are four timers」 stays true if the forward-ledger sweep armed and
